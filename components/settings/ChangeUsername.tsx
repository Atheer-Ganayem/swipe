...
import { changeUsername } from "@/libs/actions/settings";
...
const ChangeUsername = () => {
  const [state, action] = useFormState(changeUsername, { error: false, message: "" });
    ...
    <form action={action} className="card w-96 bg-base-200 shadow-xl p-5 mx-auto">
      ...
      <div className="flex flex-col gap-6">
        ...
        <label className="input input-bordered flex items-center gap-2">
          ...
          <input className="w-full" type="text" placeholder="New Username" name="newName" />
        </label>
        {/* password */}
        <label className="input input-bordered flex items-center gap-2">
          <svg
        ...
        <p className="text-sm">
          Alert: you'll be logged out after you change your username, then you have to login back to
          your account.
        </p>
        <SubmitBtn text="Change Username" />
      </div>
    </form>
  );
};
